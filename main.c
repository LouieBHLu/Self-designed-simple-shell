...
void readInput(int *isInputEnd, int *isFirstLine, int *fgetsCTRLC, int *singleQuotationFinished, int *doubleQuotationFinished){
...
void parseQuote(int *singleQuotationFinished, int *doubleQuotationFinished, char marks[], int *markNum, unsigned int delList[], unsigned int *delNum){
...
void parseRed(char *redirectLine){
...
int main(){
    ...
    while(1){
        ...
        readInput(&isInputEnd, &isFirstLine, &fgetsCTRLC, &singleQuotationFinished, &doubleQuotationFinished);
        ...
        parseQuote(&singleQuotationFinished, &doubleQuotationFinished, marks, &markNum, delList, &delNum);
         ...
         */
        int *pipeFile = (int *)malloc(sizeof(int) * (size_t)(2 + pipeNum * 2));
        ...
        if(!isBack)
        {
            for(int i = 0; i < pipeNum + 1; i++) waitpid(pids[i], NULL, WUNTRACED);
        } 
